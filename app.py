...
st.markdown("This app uses K-Nearest Neighbors (KNN) from scikit-learn to classify weather conditions based on temperature and humidity levels.")

X = np.array([[30, 70], [25, 80], [27, 60], [31, 65], [23, 65], [28, 75]])
...
n_neighbors = st.sidebar.slider("KNN value", min_value=1, max_value=10, value=3, step=1)
...
with coll:
    ...
    colors = ["orange", "blue"]
    ax.scatter(new_weather[0, 0], new_weather[0, 1], color=colors[pred], marker="*", s=300, edgecolor="black", label=f"New day: {weather_label}", zorder=5)
